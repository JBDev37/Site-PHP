...
$recherche_personnes = $pdo->prepare('SELECT comment_author_id, comment_post_ID, comment_author, comment_date FROM wp_cendrillon_6239_comments WHERE comment_author_id = ? AND comment_author_id <> ? ORDER BY comment_date DESC');

$recherche_personnes->execute([$user_id,$user_id]);

?><legend class="legend-visite"><b><?php echo "Personnes aidées par " .$name_user ?></b></legend><?php
