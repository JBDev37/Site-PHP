...
$recherche_personnes = $pdo->prepare('SELECT comment_author_id, comment_post_ID, comment_author, comment_date FROM wp_cendrillon_6239_comments WHERE post_author_id = ? ORDER BY comment_date DESC');

$recherche_personnes->execute([$my_id]);

?><legend class="legend-visite"><b>Ils m'ont aidé</b></legend><?php
while($affiche_personnes = $recherche_personnes->fetch()){

$comment_author_id = $affiche_personnes->comment_author_id;
