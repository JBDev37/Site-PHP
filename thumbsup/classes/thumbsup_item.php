...
class ThumbsUp_Item {
	 ...
	 */
	public static function create($name, $ID_author)
	{
		try
		{
			// Attempt to create a new item
			$sth = ThumbsUp::db()->prepare('INSERT INTO '.ThumbsUp::config('database_table_prefix').'items (name, ID_author, date) VALUES (?, ?, NOW())');
			$sth->execute(array($name, $ID_author));
...
$pdo->setAttribute(PDO::ATTR_DEFAULT_FETCH_MODE, PDO::FETCH_OBJ); 
			$db->query('INSERT INTO thumbsup_items SET name = ?, ID_author = ?', [$name, $ID_author])->execute();*/
		...
		$item->id_commentaire = $name;
        $item->ID_author = $ID_author;
